...
st.header("Determining Confidence level of Reliability System")
...
with holder_box_container.container():
    ...
    reliability_input = col1.slider("Desired Reliability:",min_value=0.00,max_value=1.00,value=0.85)
...
with holder_text_container.container():
    if holder_help == True:      
        ...
        st.markdown("- *Desired Reliability*: Desired Reliability: Desired posterior reliability for the system being tested.")
...
if button:
    ...
    CL = 1-beta.cdf(reliability_input,a1,b1)
    CL = round(CL*100,2)
    st.markdown(f"#### Confidence Level: {CL}%")